$(document).ready(function()
{
    ...
    $(document).on('click' , '.btn_add' , function()
    {
        ...
        let trIncreament = trCount > 0 ? parseInt($('#order-details').find('tr.cloning_row:last').attr('id')) + 1 : 0 ;

        $('#order-details').find('tbody').append($('' +
            '<tr class="cloning_row" id="' + trIncreament + '">'+
                '<td><button type="button" class="btn btn-danger btn-sm new_product"><i class="fa fa-close"></i></button></td>'+
                '<td>'+
                    '<input type="text" name="product_name[' + trIncreament + ']" class="product_name form-control">'+
                '</td>'+
                '<td>'+
                    '<input type="number" step="0.01" name="quantity[' + trIncreament + ']" class="quantity form-control">'+
                '</td>'+
                '<td>'+
                    '<input type="number" step="0.01" name="unit_price[' + trIncreament + ']" class="unit_price form-control">'+
                '</td>'+
                '<td>'+
                    '<input type="number" step="0.01" name="product_total[' + trIncreament + ']" class="product_total form-control"  readonly="readonly">'+
                '</td>'+
            '</tr>'
        ))
    });
    ...
    $('form').on('submit' , function(e)
    {
        ...
        $('input.product_total').each(function() { $(this).rules('add' , { required:true }) }) ,
        e.preventDefault();
    });
